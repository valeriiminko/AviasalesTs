...
const Filter:React.FC<Props> = ({isActive, setActive} : Props) => {
    const {active, currency} = usedTypedSelector(state => state.currencyReducer);
    const {stops, currentstop} = usedTypedSelector(state => state.stopsReducer);
                    ...
                    <div className="card-body">
                        ...
                        <p>ВАЛЮТА</p>
                            ...
                            {ARR_CURRENCY.map((item, inx) =>{
                                    return(
                                    <button type="button" className={item === currency ? 'btn btn-primary active' : 'btn btn-light'}
                                                    id={item === currency ? '1' : 'bd'} key={inx}
                                                value={item} onClick={(e) => dispatch(onSortCurrency(e, currency, active))}
                                                >
                                                    {item}
                                    </button>)
                                })}
                            ...
                            {ARR_STOPS.map((item, inx) =>{
                                    ...
                                    return(
                                        <div className="custom-control custom-checkbox">
                                            <input type="checkbox" className="custom-control-input"  value={stop} 
                                            key={inx}
                                            name={stop} 
                                            ...
                                            />
                                            <label className="custom-control-label" htmlFor={stop}>{stop}</label>
                                    </div>
                                    )
                                })}
                        </div>
