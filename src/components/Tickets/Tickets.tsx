...
const Tickets: React.FC<Props> = ({ isActive, setActive }: Props) => {
  const isActiveArr = usedTypedSelector((state) =>
    state.modalHandleReducer.tickets.filter((ticket) => ticket.isActive)
  );
  ...
  const ticketsData = usedTypedSelector((state) => {
    ...
    if (
      stopsData.every((item) => item.checked) ||
      stopsData.every((item) => !item.checked)
    ) {
      return tickets;
    }
  ...
  return (
    <div>
      {ticketsData.map((ticket, inx) => {
        return (
          <div className="card-wrapper onblur" key={inx}>
